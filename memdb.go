...
type MemDB struct {
	schema  *DBSchema
	root    unsafe.Pointer // *iradix.Tree[any] underneath
	primary bool
...
func NewMemDB(schema *DBSchema) (*MemDB, error) {
	...
	db := &MemDB{
		schema:  schema,
		root:    unsafe.Pointer(iradix.New[any]()),
		primary: true,
	}
...
func (db *MemDB) getRoot(clone bool) *iradix.Tree[any] {
	root := (*iradix.Tree[any])(atomic.LoadPointer(&db.root))
	if clone {
		root = root.Clone()
	}
	return root
...
func (db *MemDB) Txn(write bool) *Txn {
	...
	txn := &Txn{
		db:      db,
		write:   write,
		rootTxn: db.getRoot(false).Txn(write),
	}
	return txn
}
